...
export async function generateBrandKitZip(
  ...
  if (data.businessCard) {
    if (data.businessCard.frontUrl) {
      promises.push(
        fetchAsUint8Array(data.businessCard.frontUrl).then((bytes) => {
    ...
    if (data.businessCard.backUrl) {
      promises.push(
        fetchAsUint8Array(data.businessCard.backUrl).then((bytes) => {
  ...
  if (data.brandedBackdrops) {
    if (data.brandedBackdrops.feedUrl) {
      promises.push(
        fetchAsUint8Array(data.brandedBackdrops.feedUrl).then((bytes) => {
    ...
    if (data.brandedBackdrops.storyUrl) {
      promises.push(
        fetchAsUint8Array(data.brandedBackdrops.storyUrl).then((bytes) => {
    ...
    for (const f of data.favicons) {
      if (f.url) {
        promises.push(
          ...
          }).catch(e => console.error(`Failed to resize favicon ${f.size}`, e))
        );
      }
    }
  }
  ...
  if (failedCount > 0) {
    console.warn(`[download-kit] ${failedCount} files failed to generate for the ZIP.`);
    toast.warning(`${failedCount} assets could not be generated and will be missing from the ZIP.`);
  }
...
export async function downloadBrandKit(data: BrandKitResultsData) {
  try {
    ...
    toast.error("There was an error generating your brand kit zip. Please try again.");
  }
}
