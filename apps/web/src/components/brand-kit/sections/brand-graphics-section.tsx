...
export function BrandGraphicsSection({ data }: BrandGraphicsSectionProps) {
  ...
  const isTargeted = (itemId: string) =>
    targetSectionId === "brand-graphics" &&
    (!targetItemId || targetItemId === itemId);
  const isRefiningAll = refiningSectionId === "brand-graphics" && !targetItemId;
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        {graphics.map((graphic) => (
          <AssetCard
            ...
            isPlaceholder={
              graphic.url.includes("placehold.co") || Boolean(refiningSectionId)
            }
          ...
          >
            ...
            >
              <SectionContent
                sectionId="brand-graphics"
                targetItemId={isRefiningAll ? undefined : graphic.id}
